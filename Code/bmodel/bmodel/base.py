...
#        initial_condition=None,
        ...
        """
        Perturbe n_runs steady states by switching a node, but others can be updated.
        initial_condition are given by random sampling.
...
#        assert initial_condition is not None
        ...
        for _ in range(n_runs):
            ...
            initial_condition = np.empty(0, dtype=np.float64)
            # set neutral value 0 to indicator nodes  将中性值 0 设置为指标节点
#            if self._indicator_idx:
#                initial_condition = np.random.choice(
#                    np.array([-1., 1.]),
#                    size=self.N
...
#                initial_condition[self._indicator_idx] = 0
            ...
            ## 2022.10.6 Lingyu Li  add  产生-1,1的self.N维的随机向量
            initial_condition = np.random.choice(
                np.array([-1., 1.]),
                size=self.N
            )   
