...
bmodel.perturbelly(label="SNAI1 ON hold True",
...
icpoint.to_csv("ResultEMTPertur/icpointEMTpos"+ str(step)+".csv", sep=",")

sspoint = DataFrame(bmodel._perturbations_ss)
sspoint.to_csv("ResultEMTPertur/sspointEMTpos"+ str(step)+".csv", sep=",")

metapoint = DataFrame(bmodel._perturbations_meta)
metapoint.to_csv("ResultEMTPertur/metapointEMTpos"+ str(step)+".csv", sep=",")

energy = DataFrame(H)
energy.to_csv("ResultEMTPertur/energyEMTpos"+ str(step)+".csv", sep=",")
